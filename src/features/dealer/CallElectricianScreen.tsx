...
export function CallElectricianScreen() {
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      {associatedElectricians.slice(0, 10).map((item) => (
        <View key={item.id} style={styles.card}>
          ...
          <View style={styles.actions}>
            <TouchableOpacity style={[styles.actionBtn, styles.whatsAppBtn]} activeOpacity={0.88} onPress={() => Linking.openURL(`https://wa.me/${item.whatsapp}`)}>
              <WhatsAppIcon />
            </TouchableOpacity>
            <TouchableOpacity style={[styles.actionBtn, styles.callBtn]} activeOpacity={0.88} onPress={() => Linking.openURL(`tel:${item.phone}`)}>
              <PhoneIcon />
            </TouchableOpacity>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  heroEyebrow: { color: 'rgba(255,255,255,0.72)', fontSize: 11, fontWeight: '800', textTransform: 'uppercase', letterSpacing: 1.1 },
