...
export function AppSettingsPage({ onBack }: { onBack: () => void }) {
  ...
  const Toggle = ({ val, onToggle }: { val: boolean; onToggle: () => void }) => (
    <TouchableOpacity style={[styles.toggle, val && styles.toggleOn]} onPress={onToggle} activeOpacity={0.8}>
    ...
    <View style={{ flex: 1, backgroundColor: theme.bg }}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <View style={[styles.card, { backgroundColor: theme.surface, borderColor: theme.border }]}>
          ...
          {[
            { label: t('pushNotifications'), sub: t('receiveAlerts'), val: notifEnabled, toggle: () => setNotifEnabled((v) => !v), icon: 'notification' as IconName },
            { label: t('darkMode'), sub: t('switchTheme'), val: darkMode, toggle: () => setDarkMode(!darkMode), icon: 'moon' as IconName },
          ].map((item, i, arr) => (
            <View key={item.label} style={[styles.row, i < arr.length - 1 && [styles.rowBorder, { borderBottomColor: theme.border }]]}>
              <View style={[styles.iconWrap, { backgroundColor: item.val ? C.primaryLight : theme.soft }]}>
                <AppIcon name={item.icon} size={20} color={item.val ? C.primary : theme.textSecondary} />
              </View>
              <View style={{ flex: 1 }}>
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontSize: 14, fontWeight: '800', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 },
