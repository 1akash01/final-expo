...
function TestimonialCard({
  ...
  return (
    <Pressable onPress={onPress} onPressIn={handlePressIn} onPressOut={handlePressOut}>
      ...
      >
        <LinearGradient colors={item.colors} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.cardTop}>
          <Animated.View style={[styles.shine, { transform: [{ translateX: shineX }, { rotate: '16deg' }] }]} />
          <Animated.View
            style={[
              styles.avatarOuter,
              {
        ...
        <View style={styles.cardBody}>
          ...
          <View style={[styles.highlightChip, darkMode ? styles.highlightChipDark : null]}>
            <Text style={[styles.highlightText, darkMode ? styles.highlightTextDark : null]} numberOfLines={1}>
              {item.highlight}
            </Text>
          </View>
        </View>
...
function CloseIcon({ color = '#0F172A' }: { color?: string }) {
  return (
    <Text style={{ color, fontSize: 18, fontWeight: '900', lineHeight: 18 }}>x</Text>
  );
}
  ...
  return (
    <Modal transparent animationType="fade" visible onRequestClose={onClose}>
      <View style={styles.modalBackdrop}>
        ...
        >
          <Pressable style={[styles.closeBtn, darkMode ? styles.closeBtnDark : null]} onPress={onClose}>
            <CloseIcon color={darkMode ? '#E2E8F0' : '#0F172A'} />
          </Pressable>

          <View style={styles.detailHeader}>
            <LinearGradient colors={item.colors} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.detailAvatarFrame}>
            ...
            <View style={styles.detailInfo}>
              <Text style={[styles.detailName, darkMode ? styles.detailNameDark : null]}>{item.name}</Text>
              <Text style={[styles.detailMeta, darkMode ? styles.detailMetaDark : null]}>{item.location}</Text>
              <View style={styles.detailChipRow}>
                <View style={[styles.detailChip, darkMode ? styles.detailChipDark : null]}>
                  <Text style={[styles.detailChipText, darkMode ? styles.detailChipTextDark : null]}>{item.tier}</Text>
                </View>
                <View style={[styles.detailChip, darkMode ? styles.detailChipDark : null]}>
                  <Text style={[styles.detailChipText, darkMode ? styles.detailChipTextDark : null]}>{item.yearsWithUs}</Text>
                </View>
              </View>
            </View>
          </View>

          <Text style={[styles.detailQuote, darkMode ? styles.detailQuoteDark : null]}>"{item.quote}"</Text>

          <View style={[styles.detailHighlight, darkMode ? styles.detailHighlightDark : null]}>
            <Text style={[styles.detailHighlightText, darkMode ? styles.detailHighlightTextDark : null]}>
              {item.highlight}
            </Text>
          </View>
        </Animated.View>
    ...
    <View style={[styles.section, darkMode ? styles.sectionDark : null]}>
      ...
      >
        ...
        <View style={styles.rail}>
          <LinearGradient
            colors={darkMode ? ['#60A5FA', '#38BDF8'] : ['#2563EB', '#60A5FA']}
            start={{ x: 0, y: 0.5 }}
            end={{ x: 1, y: 0.5 }}
            style={[styles.railFill, { width: `${Math.max(26, ((selectedIndex + 1) / Math.max(items.length, 1)) * 100)}%` }]}
          />
        </View>
      </LinearGradient>
      {openedItem ? <DetailCard item={openedItem} darkMode={darkMode} onClose={() => setOpenIndex(null)} /> : null}
    </View>
  );
}
